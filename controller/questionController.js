...
const {
	test
} = require('../Models/candidateAnswer')
const {
	examDetail
} = require('../Models/examDetail')
const {
	checkExistingRightOption,
	checkExistingWrongOption,
	radioOrCheckBoxValue
} = require('./questionLogic')
...
const testQuestions = async(req, res) => {
	...
	let ques = await questionDetail.find({
		'examCode': req.headers.examcode
	}).skip(pageNumber).limit(1).select({
		"questionImage": 1,
		"questionText": 1,
		"option1": 1,
		"option2": 1,
	...
	let lastQuestion = await questionDetail.find({
		'examCode': req.headers.examcode
	}).select({
		"questionText": 1
	})
	...
	const time = await examDetail.find({
		'examCode': req.headers.examcode
	}).select({
		examName: 1,
		examStartTime: 1,
...
const saveCorrectOption = async(req,checkAnswer,existingAnswer)=>{
    ...
    if(existingAnswer === null){
		...
		let existingScore = await test.findOne({ $and: [{ candidateId: req.headers.id }, { testCode: req.body.code }] }).select({totalScore:1})
...
const checkAccessKey = async(req, res) => {
	const status = await examDetail.find({
...
const saveAllQuestions = async(req, res) => {
	const allQuestions = await questionDetail.find({
		examCode: req.headers.examcode
	}).select({
		_id: 1
	})
	const savedQuestions = await test.findOne({
		$and: [{
			candidateId: req.headers.id
		}, {
			testCode: req.headers.examcode
		}]
	})
...
const getExamTime = async(req, res) => {
	const examData = await examDetail.findOne({
		examCode: req.headers.examcode
	})
	const submitStatus = await test.findOne({
		$and: [{
			candidateId: req.headers.id
		}, {
			testCode: req.headers.examcode
	...
	if (submitStatus === null) {
		...
		res.status(200).send({
			examData: examData,
			submitStatus: submitStatus.submitExam
		})
	}
...
const fetchQuestionById = async(req, res) => {
	try {
		let obj = await questionDetail.findById({
			_id: req.params.id
		})
